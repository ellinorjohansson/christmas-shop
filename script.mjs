...
function updateCart() {
  ...
  document.querySelector("#cart-total-header").innerText =
    "Total: " + totalAfterDiscount.toFixed(2) + " SEK";

  //Show shipping cost in overlay
  document.querySelector("#cart-total").innerText =
    totalWithShipping.toFixed(2) + " SEK";
...
function calculateShipping() {
  ...
  document.querySelector("#shipping-cost").innerText =
...
function handlePaymentMethodChange() {
  ...
  const personalNumberContainer = document.getElementById(
...
function generateStar(rating) {
  ...
  for (let i = 0; i < 5; i++) {
    if (i < rating) {
      stars +=
        '<span class="material-symbols-outlined rating-star">star</span>'; // Filled star
    } else {
      stars +=
        '<span class="material-symbols-outlined rating-star-empty">star</span>'; // Empty star
    }
  }
  return stars;
}
  ...
  increaseButtons.forEach((button) => {
    button.addEventListener("click", function () {
      const quantitySpan = document.querySelector(
        `.quantity[data-name="${button.getAttribute("data-name")}"]`,
      );
...
function setupAddToCartButtons() {
  ...
  addItemButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      const productName = button.getAttribute("data-name");
      const quantitySpan = document.querySelector(
        `.quantity[data-name="${productName}"]`,
      );
  ...
  function validateForm() {
    ...
    if (firstnameInput.value.trim() === "") {
      firstnameError.innerHTML =
        '<span class="material-symbols-outlined">error</span> Firstname is required.';
    ...
    if (lastnameInput.value.trim() === "") {
      lastnameError.innerHTML =
        '<span class="material-symbols-outlined">error</span> Lastname is required.';
    ...
    if (!zipRegex.test(zipInput.value)) {
      zipError.innerHTML =
        '<span class="material-symbols-outlined">error</span> Invalid ZIP code. Enter a 5-digit code.';
    ...
    if (cityInput.value.trim() === "") {
      cityError.innerHTML =
        '<span class="material-symbols-outlined">error</span> City is required.';
    ...
    if (!phoneRegex.test(phoneInput.value)) {
      phoneError.innerHTML =
        '<span class="material-symbols-outlined">error</span> Invalid phone number!';
    ...
    if (!emailRegex.test(emailInput.value)) {
      emailError.innerHTML =
        '<span class="material-symbols-outlined">error</span> Invalid mail! Enter a correct mail.';
    ...
    if (!isCardSelected) {
      ...
      if (!personalNumberRegex.test(personalNumberInput.value)) {
        personalNumberError.innerHTML =
          '<span class="material-symbols-outlined">error</span> Invalid personal number! Enter in: YYMMDD-XXXX.';
    ...
    if (!termsCheckbox.checked) {
      termsError.innerHTML =
        '<span class="material-symbols-outlined">error</span> You must accept the terms.';
